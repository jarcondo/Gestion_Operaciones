...
namespace Intranet.Web.Sistema.Modulos.ProduccionServicios.Reportes.ProgramacionOT
{
    public partial class ProgramacionCuadrilla : BasePage
    {
        ...
        private void CargarCombos()
        {
            try
            {
                ...
                //List<EstadoOTDTO> olEstado = new EstadoOTBL().ListarEstadoOT(); //(List<EstadoOTDTO>)Session["session.estadoOT.intranet"];
        ...
        private void CargaCuadrilla()
        {
            try
            {
                ...
                olCuadrilla.Insert(1, new CuadrillaDTO() { IdCuadrilla = 1000000, Descripcion = "SIN CUADRILLA" });
        ...
        protected void CargarReporte(object sender, DirectEventArgs e)
        {
            try
            {
                if (this.ddlObra.SelectedIndex >= 0)
                {
                    ...
                    string tipoFecha = "";
                    if (this.Radio1.Checked == true) tipoFecha = "PROGRAMADA";
                    if (this.Radio2.Checked == true) tipoFecha = "INICIO";

                    string sinCuadrilla = "";
                    if (this.ddlCuadrilla.SelectedItem.Value == "1000000") sinCuadrilla = "1"; else sinCuadrilla = "0";

                    List<ProgramacionCuadrillaDTO> olOTProgramacion = new List<ProgramacionCuadrillaDTO>();
                    olOTProgramacion = new EjecucionOTBL().ObtenerProgramacionCuadrilla(obra, cuadrilla, tipo, estado, fDesde, fHasta, area, tipoFecha, sinCuadrilla);

                    if (chkPuntaje.Checked)
                    {
                        Session["Intranet.VisorReporte.Ruta"] = "~/Sistema/Modulos/ProduccionServicios/Reportes/ProgramacionOT/rptEjecucionOTPuntaje.rpt";
                    }
                    else
                    {
                        Session["Intranet.VisorReporte.Ruta"] = "~/Sistema/Modulos/ProduccionServicios/Reportes/ProgramacionOT/rptEjecucionOT.rpt";
                    }
                    ...
                    LoadConfig config = new LoadConfig("../VisorReporte.aspx", LoadMode.IFrame);

                    winPDF.LoadContent(config);
            ...
            try
            {
                if (this.ddlObra.SelectedItem.Value != "0")
                {
                    ...
                    else
                    {
                        ...
                        listaCuadrilla.Insert(1, new CuadrillaDTO() { IdCuadrilla = 1000000, Descripcion = "SIN CUADRILLA" });
